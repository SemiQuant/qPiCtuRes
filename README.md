...
`qPiCtuRes` is published as the [`qpictures`](https://pypi.org/project/qpictures/)
distribution on PyPI. The repo is set up for **trusted publishing** (OIDC,
no API token required) via GitHub Actions.

### One-time setup

1. Push the repo to `https://github.com/SemiQuant/qPiCtuRes` (or update the
   URLs in `pyproject.toml` and the workflow files if you fork it).
2. On PyPI, go to **Account → Publishing → Add a new pending publisher** and
   register a GitHub publisher with:
   - Owner: `SemiQuant`
   - Repository: `qPiCtuRes`
   - Workflow: `publish.yml`
   - Environment: `pypi`
3. Repeat on [TestPyPI](https://test.pypi.org/) with environment `testpypi`
   if you want a staging upload path.
...
# 2) Smoke-build locally
pip install -e ".[dev]"
python -m build
python -m twine check --strict dist/*

# 3) (Optional) Upload to TestPyPI manually first
python -m twine upload --repository testpypi dist/*

# 4) Tag and push — the publish workflow uploads to PyPI on tags
git commit -am "release: v0.2.0"
git tag v0.2.0
git push origin main --tags
```

The `Publish to PyPI` workflow (`.github/workflows/publish.yml`) also accepts a
manual `workflow_dispatch` run with a `testpypi` / `pypi` target — handy when
you want a TestPyPI build without tagging.

### Versioning

The version lives in **one place** — `pyproject.toml` — and `qpictures.__version__`
reads it back via `importlib.metadata.version("qpictures")`.
